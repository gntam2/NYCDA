...
moby_paragraph = "It will be seen that this mere painstaking burrower and grub-worm of
...
unsplinterable glasses!".downcase 

total_words = moby_paragraph.count(' ') #test to see if words are accounted for

words = moby_paragraph.split(" ")
frequencies = Hash.new(0)
words.each { |word| frequencies[word] += 1 }

frequencies.each { |word, frequency| puts word + " " + frequency.to_s }

#pull the max value from frequency and display that and word
#I am thinking a loop that checks each value
#or even rying a max value for frequency.to_s // not working with max 